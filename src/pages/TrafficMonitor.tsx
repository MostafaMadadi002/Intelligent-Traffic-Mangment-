      ...
      <header className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 py-6 border-b border-white/10">
        <div className="flex flex-col gap-1">
          <h2 className="text-2xl md:text-3xl font-light text-white leading-tight">
            {language === 'fa' ? 'پایش ' : 'Live '}
            <span className="font-bold">{language === 'fa' ? 'عصبی زنده' : 'Neural Monitoring'}</span>
          </h2>
          <p className="text-slate-500 text-[10px] md:text-sm uppercase tracking-widest font-bold">
            {language === 'fa' ? 'گره‌های تشخیص اشیاء در لحظه.' : 'Real-time object detection nodes.'}
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-stretch md:items-center gap-4">
          ...
          <div className="flex p-1 rounded-2xl bg-white/5 border border-white/10 overflow-x-auto no-scrollbar">
            ...
            <select 
              ...
              className={`bg-transparent text-[10px] md:text-xs font-bold text-slate-300 px-3 py-2 outline-none cursor-pointer hover:text-white transition-colors uppercase tracking-widest ${language === 'fa' ? 'border-r' : 'border-l'} border-white/10 min-w-[120px]`}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        <div className="lg:col-span-2 space-y-6">
            ...
            {selectedCamera ? (
              <>
                ...
                <div className="absolute top-4 left-4 md:top-6 md:left-6 flex items-center gap-2 bg-slate-950/80 backdrop-blur-md px-3 md:px-4 py-1.5 md:py-2 rounded-full border border-white/10">
                  ...
                  <span className="text-white text-[9px] md:text-[10px] font-black tracking-[0.2em] uppercase whitespace-nowrap">
                    {language === 'fa' ? 'سیگنال زنده' : 'Live Signal'}
                  </span>
                </div>
                
                <div className={`absolute bottom-4 ${language === 'fa' ? 'right-4' : 'left-4'} md:bottom-6 md:${language === 'fa' ? 'right-6' : 'left-6'} flex flex-wrap gap-3 md:gap-4`}>
                  <div className="glass-dark backdrop-blur-md px-3 md:px-5 py-2 md:py-3 rounded-[1rem] md:rounded-2xl border border-white/10">
                     ...
                     <p className="text-sm md:text-xl font-bold text-white leading-none tracking-normal">{selectedCamera?.id ? selectedCamera.id.split('-')[0] : '---'}</p>
                  </div>
                  <div className="glass-dark backdrop-blur-md px-3 md:px-5 py-2 md:py-3 rounded-[1rem] md:rounded-2xl border border-white/10">
                     <p className="text-[8px] md:text-[10px] text-slate-500 font-black uppercase tracking-widest mb-0.5 md:mb-1 leading-none">{t('density')}</p>
                     <p className="text-sm md:text-xl font-bold text-cyan-400 leading-none tracking-normal">{detection?.density || 0}%</p>
                  </div>
                </div>

                <button className={`absolute bottom-4 ${language === 'fa' ? 'left-4' : 'right-4'} md:bottom-6 md:${language === 'fa' ? 'left-6' : 'right-6'} p-2 md:p-3 glass-dark hover:bg-white/10 rounded-xl md:rounded-2xl transition-all text-white border border-white/10 group`}>
              ...
              <div className="flex flex-col items-center justify-center h-full text-slate-500 space-y-4">
                ...
                <p className="font-medium">{language === 'fa' ? 'گرهی انتخاب نشده است' : 'No system node selected'}</p>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: t('cars'), count: detection?.vehicleCounts?.car || 0, icon: Car, color: 'cyan' },
              { label: t('buses'), count: detection?.vehicleCounts?.bus || 0, icon: Bus, color: 'indigo' },
              { label: t('trucks'), count: detection?.vehicleCounts?.truck || 0, icon: Truck, color: 'slate' },
              { label: t('bikes'), count: detection?.vehicleCounts?.motorcycle || 0, icon: Bike, color: 'emerald' },
            ].map(cls => (
              <div key={cls.label} className="glass p-5 rounded-2xl border border-white/5 hover:bg-white/10 transition-colors group">
                <div className="flex justify-between items-start mb-2">
                   ...
                   <span className="text-2xl font-black text-white font-mono tracking-normal">{cls.count}</span>
                </div>
        ...
        <div className="space-y-6">
          <div className="glass rounded-3xl p-8 border border-white/5">
            <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.2em] mb-8 font-mono flex items-center gap-2">
              ...
              {language === 'fa' ? 'تحلیل استریم' : 'Stream Analytics'}
            </h3>
            <div className="grid grid-cols-2 gap-4 mb-8">
               <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">Codec</p>
                  <p className="text-xs font-bold text-white font-mono tracking-normal">{streamInfo?.metadata?.codec || 'H.264'}</p>
               </div>
               <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">FPS</p>
                  <p className="text-xs font-bold text-white font-mono tracking-normal">{streamInfo?.metadata?.fps || 30}</p>
               </div>
               <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">Bitrate</p>
                  <p className="text-xs font-bold text-cyan-400 font-mono tracking-normal">{streamInfo?.metadata?.bitrate || '0.0 Mbps'}</p>
               </div>
               <div className="bg-white/5 p-3 rounded-xl border border-white/5">
                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">Res</p>
                  <p className="text-xs font-bold text-white font-mono tracking-normal">{streamInfo?.metadata?.resolution || '1080p'}</p>
               </div>
            </div>
            
            <div className="space-y-8">
              {[
                { name: language === 'fa' ? 'تراکم اصلی' : 'Core Density', value: detection?.density || 10, color: 'cyan' },
                { name: language === 'fa' ? 'جریان محیطی' : 'Peripheral Flow', value: Math.max(0, (detection?.density || 20) - 10), color: 'indigo' },
                { name: language === 'fa' ? 'لود کمکی' : 'Auxiliary Load', value: Math.max(0, (detection?.density || 30) - 20), color: 'slate' },
              ].map(lane => (
                <div key={lane.name} className="space-y-3">
                  <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-slate-500">
                    <span>{lane.name}</span>
                    <span className="text-white font-mono tracking-normal">{lane.value}%</span>
                  </div>
          ...
          <div className="glass-dark rounded-3xl p-8 border border-white/5 relative overflow-hidden">
            ...
            <h3 className="text-xs font-black text-slate-500 uppercase tracking-[0.3em] mb-6 relative z-10">
              {language === 'fa' ? 'دفتر وقایع' : 'Event Ledger'}
            </h3>
            <div className="space-y-6 relative z-10">
              {(Array.isArray(history) ? history : []).map((log, i) => log && (
                <motion.div
                  ...
                  key={log.timestamp}
                  className={`border-${isRtl ? 'r' : 'l'}-2 border-cyan-500/30 ${isRtl ? 'pr' : 'pl'}-4 py-1 transition-opacity duration-500`}
                >
                  <p className="text-[10px] font-mono text-cyan-400/60 uppercase tracking-widest tracking-normal">{new Date(log.timestamp).toLocaleTimeString([], {hour12: false})}</p>
                  <p className="text-sm font-semibold text-white">
                    {language === 'fa' ? 'تغییرات تشخیص: ' : 'Detection Delta: '} 
                    +{Object.values((log?.vehicleCounts || {}) as Record<string, number>).reduce((a, b) => (Number(a) || 0) + (Number(b) || 0), 0)} units
                  </p>
                </motion.div>
              ))}
            </div>
