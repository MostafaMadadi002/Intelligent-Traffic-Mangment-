  ...
  return (
    <div className="space-y-6 md:space-y-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row md:items-center justify-between mb-4 md:mb-8 pb-6 border-b border-white/5 relative gap-6">
        ...
        <div>
          <h2 className="text-3xl md:text-4xl font-light text-white tracking-tight italic leading-tight">
            {language === 'fa' ? 'ماتریکس ' : 'Urban '}
            <span className="font-black not-italic text-cyan-400">
              {language === 'fa' ? 'کنترل شهری' : 'Control Matrix'}
            </span>
          </h2>
          <p className="text-slate-500 text-[10px] md:text-sm mt-1 uppercase tracking-widest font-bold font-mono">
            {language === 'fa' ? 'مرکز فرماندهی عملیات [بخش ۰۴]' : 'Operations Command Center [Sector 04]'}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-4 md:gap-6">
          ...
          <div className="glass-dark border border-white/10 px-4 md:px-6 py-2 md:py-3 rounded-2xl flex flex-col items-center justify-center">
            ...
            <span className="text-[10px] md:text-xs font-bold font-mono text-white tracking-normal">{new Date().toLocaleTimeString([], { hour12: false, hour: '2-digit', minute: '2-digit' })}</span>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {[
          { label: t('totalVehicles'), value: stats.totalVehicles, icon: Car, color: 'cyan' },
          { label: t('averageDensity'), value: `${stats.avgDensity.toFixed(1)}%`, icon: Activity, color: 'emerald' },
          { label: t('activeCameras'), value: stats.activeCameras, icon: Users, color: 'indigo' },
          { label: t('alerts'), value: stats.congestionAlerts, icon: AlertTriangle, color: 'amber' }
        ].map((item, i) => (
          <motion.div
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-white tracking-normal">{item.value}</p>
              </div>
            </div>
          </motion.div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 md:mb-8 relative z-10 mb-6 px-1">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg md:text-xl font-bold truncate">
                  {language === 'fa' ? 'نمای کلی تقاطع‌ها' : 'Intersection Overview'}
                </h3>
              </div>
              <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto">
                 <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)]" />
                    <span className="text-[9px] md:text-[10px] uppercase font-bold text-slate-500 whitespace-nowrap">
                      {language === 'fa' ? 'پرترافیک' : 'Congested'}
                    </span>
                 </div>
                 <div className="h-4 w-[1px] bg-white/10" />
                 <button className="text-[10px] md:text-xs font-black uppercase tracking-widest text-cyan-400 hover:text-cyan-300 transition-colors whitespace-nowrap">
                   {language === 'fa' ? 'همزاد دیجیتال' : 'Digital Twin'}
                 </button>
              </div>
            </div>
                  ...
                  >
                    <div className="flex items-center gap-4 flex-1 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2 flex-wrap mb-2">
                          <p className="text-[9px] md:text-[10px] uppercase font-bold tracking-widest text-slate-500 truncate">{camera.location}</p>
                          ...
                          <p className={`text-[9px] md:text-[10px] font-black uppercase whitespace-nowrap ${cameraStats[camera.id]?.density > 70 ? 'text-red-400' : 'text-emerald-400'}`}>
                            {cameraStats[camera.id]?.density || 0}% {t('density')}
                          </p>
                        </div>
                    ...
                    <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center pt-3 sm:pt-0 border-t sm:border-t-0 border-white/5">
                      <div className="flex flex-col items-start sm:items-end">
                        <p className="text-sm md:text-base font-mono font-bold text-white leading-none tracking-normal">{cameraStats[camera.id]?.flow || 0}</p>
                        <p className="text-[8px] uppercase font-bold text-slate-500 tracking-tighter mt-1">
                          {language === 'fa' ? 'وسیله/دقیقه' : 'Vehicles/Min'}
                        </p>
                      </div>
                    </div>
                  </motion.div>
        ...
        <div className="space-y-6">
          <div className="glass-dark rounded-3xl p-8 text-white relative overflow-hidden group border border-white/5 shadow-2xl">
            <div className="absolute top-0 right-0 w-32 h-32 bg-cyan-500/20 rounded-full blur-2xl group-hover:bg-cyan-500/30 transition-colors" />
            <div className="relative z-10 space-y-6">
              ...
              <div>
                <h3 className="text-xl font-bold leading-tight mb-2">
                  {language === 'fa' ? 'بهینه‌سازی هوش مصنوعی' : 'AI Optimization'}
                </h3>
                <p className="text-slate-400 text-sm leading-relaxed">
                  {language === 'fa' 
                    ? 'کنترل‌کننده‌های هوشمند در ۱۲ بخش فعال هستند و زمان انتظار را تا ۲۲٪ کاهش داده‌اند.'
                    : 'Smart controllers are active across 12 sectors, reducing idle times by 22% since calibration.'}
                </p>
              </div>
              <button 
              ...
              >
                {language === 'fa' ? 'کالیبراسیون مجدد' : 'Recalibrate System'}
              </button>
            </div>
          </div>

          <div className="glass rounded-3xl p-8">
            <h3 className="text-sm font-black uppercase tracking-widest text-slate-500 mb-6 font-mono">
              {language === 'fa' ? 'وضعیت زیرساخت' : 'Infrastructure Status'}
            </h3>
            <div className="space-y-6">
              {[
                { label: language === 'fa' ? 'سلامت سیستم' : 'System Health', status: language === 'fa' ? 'بهینه' : 'Optimal', color: 'emerald' },
                { label: language === 'fa' ? 'تاخیر شبکه' : 'Network Latency', status: '24ms', color: 'cyan' },
                { label: language === 'fa' ? 'پایداری تصویر' : 'Vision Uptime', status: '99.8%', color: 'indigo' }
              ].map(status => (
                <div key={status.label} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-white font-mono tracking-normal">{status.status}</span>
                  </div>
                </div>
              ))}
            </div>
